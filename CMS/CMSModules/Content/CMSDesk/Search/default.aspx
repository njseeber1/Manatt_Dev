        ...
        function SelectItem(nodeId, culture) {
            if (nodeId != 0) {
                var origin = window.location.protocol + "//" + window.location.hostname + (window.location.port ? ':' + window.location.port : ''),
                    newHref = origin + "<%= SystemContext.ApplicationPath %>" + "/Admin/CMSAdministration.aspx/default.aspx?action=edit&nodeid=" + nodeId + "&culture=" + culture + "<%= CMS.PortalEngine.UIContextHelper.GetApplicationHash("cms.content", "content") %>",
