function PredictiveSearchExtender(clientID, textBoxID, resultsHolderID, minChars, selectionEnabled, selectedCssClass, resultsCssClass) {
    ...
    this.ScrollToView = function (item) {
        var $activeMenu = item.parent();
        var borderTop, paddingTop, offset, scroll, elementHeight, itemHeight;
        borderTop = parseFloat($.css($activeMenu[0], "borderTopWidth")) || 0;
        paddingTop = parseFloat($.css($activeMenu[0], "paddingTop")) || 0;
        offset = item.offset().top - $activeMenu.offset().top - borderTop - paddingTop;
    ...
    this.MoveSelectedDown = function () {
        ...
        if (searchResultsDiv.is(':visible')) { // results are visible - navigate results
            if (me.selectedResult + 1 <= searchResults.length - 1) { // result is not the last
                ...
                me.ScrollToView(searchResults.eq(me.selectedResult));
        ...
        if (query.length >= me.minChars) { // query is long enough
            if (query !== me.lastSearchQuery) // query is different from the last query
            {
                me.timeout = setTimeout(function () {
                    //var tmpVS = document.getElementById("__VIEWSTATE").value;
                    //$cmsj('#__VIEWSTATE').remove();

                    //me.CallPredictiveSearch(query, query);
                    var predictiveUrl = window.location.pathname;
                    if ($cmsj('.form-inline.search-form.hovered').length > 0)
                        predictiveUrl = "/search";

                    $cmsj.ajax({
                        type: "GET",
                        contentType: "application/json;charset=utf-8",
                        url: predictiveUrl + "?autocomplete=" + query,
                        
                        success: function (msg) {
                            me.RecieveSearchResults(msg, query);
                        }
                    });

                }, 250); // set call timeout (start the search countdown)
            }
