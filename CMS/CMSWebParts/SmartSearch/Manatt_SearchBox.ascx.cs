...
/// </summary>
public partial class CMSWebParts_SmartSearch_Manatt_SearchBox : CMSAbstractWebPart, ICallbackEventHandler
{
    #region "Variables"
    ...
    protected override void OnLoad(EventArgs e)
    {
        ...
        if (Request.QueryString["autocomplete"] != null && Request.QueryString["autocomplete"] != "")
        {
            RaiseCallbackEvent(Request.QueryString["autocomplete"].ToString());
        }
    }
    ...
    private static Dictionary<string, string> urls = new Dictionary<string, string>()
    {
        ...
        { "News", "/insights/news" },
        { "Awards", "/insights/awards" },
        { "Events", "/insights/events" },
        { "White Papers", "/insights/white-papers" },
        { "CLEs", "/insights/CLEs" },
        { "Webinars", "/insights/webinars" },
        { "Careers", "/careers/current-opportunities" },
        { "The Firm", "/search/firm" }
    };
    /// <summary>
    /// Performs search.
    /// </summary>
    ///     
    private DataSet PredictiveSearch(string searchText)
    {
        var indexes = CMS.Search.SearchIndexInfoProvider.GetSearchIndexes(); //"ServicesIndex;PeopleIndex;Experience;WhitePapers;Webinars;Newsletters;Articles;News;Awards;Events;CLEs;CareersIndex;TheFirm";

        DataSet results = new DataSet();
        results.Tables.Add("results");
        results.Tables[0].Columns.Add("id");
        results.Tables[0].Columns.Add("type");
        results.Tables[0].Columns.Add("score");
        results.Tables[0].Columns.Add("position");
        results.Tables[0].Columns.Add("title");
        results.Tables[0].Columns.Add("content");
        results.Tables[0].Columns.Add("created");
        results.Tables[0].Columns.Add("image");
        results.Tables[0].Columns.Add("imageclass");
        results.Tables[0].Columns.Add("absscore");
        results.Tables[0].Columns.Add("maxscore");
        results.Tables[0].Columns.Add("index");        
        results.Tables[0].Columns.Add("NodeAliasPath");        
        ...
        var lstPredictiveSearchIndexes = PredictiveSearchIndexes.Split(';').ToList();

        foreach (var index in indexes) //indexes.Split(';').Skip(2)) //PredictiveSearchIndexes.Split(';').Skip(2))
        {
            if (index.IndexName == "ServicesIndex" || index.IndexName == "PeopleIndex" || !lstPredictiveSearchIndexes.Contains(index.IndexName))
                continue;
            ...
            SearchParameters parameters = new SearchParameters()
            {
                ...
                SearchIndexes = index.IndexName, // index, //PredictiveSearchIndexes,
                StartingPosition = 0,
                DisplayResults = 5, //6, //PredictiveSearchMaxResults,
                NumberOfProcessedResults = 100, //100 > PredictiveSearchMaxResults ? PredictiveSearchMaxResults : 100,
                NumberOfResults = 0,
            ...
            // Search       
            var tmpResult = SearchHelper.Search(parameters);
            if (tmpResult != null)
            {
                //foreach (DataRow row in tmpResult.Tables[0].Rows)
                //{
                //    DataRow dr = results.Tables[0].NewRow();
                //    for (int i = 0; i < 12; i++)
                //    {
                //        dr[i] = row[i].ToString();
                //    }
                //    results.Tables[0].Rows.Add(dr);
                //}
                results.Merge(tmpResult); // tmpResult.Merge(results);
            }
        }
        ...
        using (System.Data.SqlClient.SqlConnection con = new System.Data.SqlClient.SqlConnection(CMS.DataEngine.ConnectionHelper.GetSqlConnectionString()))
        {
            ...
            //Services
            if (PredictiveSearchIndexes.Contains("ServicesIndex"))
            {
                using (System.Data.SqlClient.SqlCommand cmd = new System.Data.SqlClient.SqlCommand(
                       @"select DISTINCT TOP 5 vw.documentid, vw.NodeId, vw.DocumentUrlPath, mp.Title, mp.IntroText AS Content from manatt_practice mp  
                    inner join view_cms_tree_joined vw on mp.practiceid = vw.documentforeignkeyvalue and vw.classname = 'manatt.practice' and vw.nodeLinkedNodeID is NULL
                    and (NodeAliasPath like '/Services/%' or NodeAliasPath like '/Industries/%') 
                    WHERE TITLE LIKE @SearchText + '%' OR TITLE LIKE '% ' + @SearchText + '%' 
                            OR vw.DocumentTags LIKE @SearchText + '%' OR vw.DocumentTags LIKE '% ' + @SearchText + '%' 
                            OR vw.DocumentPageKeywords LIKE @SearchText + '%' OR vw.DocumentPageKeywords LIKE '% ' + @SearchText + '%' 
                    ORDER BY mp.Title", con))
                {
                    ...
                    while (result.Read())
                    {
                        ...
                        dr[0] = result["DocumentId"].ToString() + ";" + result["NodeId"].ToString() + "_cms.document";
                        dr[1] = "cms.document";
                        dr[3] = results.Tables[0].Rows.Count;
                        dr[4] = result["Title"].ToString();
                        dr[5] = result["Content"].ToString();
                        dr[11] = "ServicesIndex";
                        dr[12] = result["DocumentUrlPath"].ToString();
                        results.Tables[0].Rows.Add(dr);
                    }
                    result.Close();
                }
            ...
            //People
            //OR p.FirstName + ' ' + p.LastName LIKE @SearchText +'%'
            //                OR p.FirstName + ' ' + p.LastName LIKE ' ' + @SearchText + '%'
            //                OR p.LastName + ' ' + p.FirstName LIKE ' ' + @SearchText + '%'
            //                OR p.LastName + ' ' + p.FirstName LIKE ' ' + @SearchText + '%'

            if (PredictiveSearchIndexes.Contains("PeopleIndex"))
            {
                using (System.Data.SqlClient.SqlCommand cmd = new System.Data.SqlClient.SqlCommand())
                {
                    cmd.Connection = con;

                    if (PredictiveSearchIndexesAlternate == "2")
                    {
                        cmd.CommandText = @"select DISTINCT TOP 5 vw.documentid, vw.NodeId, vw.DocumentUrlPath, vw.DocumentName AS Title, vw.DocumentName AS Content, 0 as SortIndex from view_cms_tree_joined vw 
                                            join manatt_people p on p.PeopleID = vw.DocumentForeignKeyValue
                                            WHERE vw.classname = 'manatt.people' and vw.nodeLinkedNodeID is NULL 
                                            AND (p.PeopleFullNames LIKE @SearchText + '%' OR p.PeopleFullNames LIKE '% ' + @SearchText + '%') 
                                            order by Title";
                    }
                    else
                    {
                        cmd.CommandText = @"select documentid, NodeId,  DocumentUrlPath, Title, Content, SortIndex from (
                                            select top 5 documentid, NodeId, DocumentUrlPath, Title, Content, min(SortIndex) as SortIndex from 
	                                            (select DISTINCT TOP 5 vw.documentid, vw.NodeId, vw.DocumentUrlPath, vw.DocumentName AS Title, vw.DocumentName AS Content, 0 as SortIndex from view_cms_tree_joined vw 
                                            join manatt_people p on p.PeopleID = vw.DocumentForeignKeyValue
                                            WHERE vw.classname = 'manatt.people' and vw.nodeLinkedNodeID is NULL 
                                            AND (DocumentName LIKE @SearchText + '%' OR DocumentName LIKE '% ' + @SearchText + '%' 
                                                    OR p.PeopleFullNames LIKE @SearchText + '%' OR p.PeopleFullNames LIKE '% ' + @SearchText + '%' 
                                                    )
                                            UNION ALL
                                            select DISTINCT TOP 5 vw.documentid, vw.NodeId, vw.DocumentUrlPath, vw.DocumentName AS Title, vw.DocumentName AS Content, 1 AS SortIndex from view_cms_tree_joined vw 
                                            join manatt_people p on p.PeopleID = vw.DocumentForeignKeyValue
                                            WHERE vw.classname = 'manatt.people' and vw.nodeLinkedNodeID is NULL 
                                            AND (
                                                    vw.DocumentPageKeywords LIKE @SearchText + '%' OR vw.DocumentPageKeywords LIKE '% ' + @SearchText + '%' 
                                                    OR p.ProfesionalExperience LIKE @SearchText + '%' OR p.ProfesionalExperience LIKE '% ' + @SearchText + '%')
                                            ) as t 
                                            group by documentid, NodeId, DocumentUrlPath, Title, Content
                                            ORDER BY min(SortIndex), Title) as t2
                                            order by Title";
                    }
                    ...
                    while (result.Read())
                    {
                        ...
                        if (result["SortIndex"].ToString() == "1")
                            drTmp = dr;
            ...
            {
                if (dr["NodeAliasPath"].ToString() == "")
                {
                    using (System.Data.SqlClient.SqlCommand cmd = new System.Data.SqlClient.SqlCommand(
                        @"select NodeAliasPath from view_cms_tree_joined
                        WHERE documentid = @DocumentId AND NodeId = @NodeId", con))
                    {
                        cmd.Parameters.AddWithValue("@DocumentId", dr["id"].ToString().Split(';')[0]);
                        cmd.Parameters.AddWithValue("@NodeId", dr["id"].ToString().Split(';')[1].Split('_')[0]);

                        var result = cmd.ExecuteScalar();
                        if (result.ToString() != "")
                            dr["NodeAliasPath"] = result.ToString();
                        else
                            dr["NodeAliasPath"] = "/";
                    }
                }
    ...
    {        
        ...
        else
        {
            ...
            // Sort category
            var indexOrder = PredictiveSearchIndexes.Split(';').ToList();
            var lstIndexCategories = indexCategories.OrderBy(x => indexOrder.IndexOf(x.Key)).ToList();

            // Render each index category
            var lines = 0;
            foreach (var categories in lstIndexCategories) //indexCategories)
            {
                var take = 4; // PredictiveSearchMaxResults - lines;
                //if (take == 0)
                //    break;
                
                //if (indexOrder.IndexOf(categories.Key) < 2)
                //{
                //    categories.Value.RowFilter += " AND Title LIKE '%" + searchText + "%'";
                ...
                if (PredictiveSearchDisplayCategories)
                {
                    ...
                    if (categories.Value.Count > 4)
                        repSearchResults.FooterTemplate = new TextTransformationTemplate("<div class='predictiveSearchViewAll'><a href=\"" + urls[categoryName] + "?searchmode=exactphrase&searchtext=" + searchText + "\">View All</a></div>");
                    else
                        repSearchResults.FooterTemplate = new TextTransformationTemplate("");
                    //if (indexOrder.IndexOf(categories.Key) < 2)
                    //    take = 4;
                }

                DataTable tmp = categories.Value.ToTable().Clone();
                for (int i = 0; i < categories.Value.ToTable().Rows.Count && i < take; i++)
                {
                    tmp.ImportRow(categories.Value.ToTable().Rows[i]);
                    lines += 1;
                }
                ...
                repSearchResults.DataSource = tmp; // categories.Value;
    ...
    private static System.Collections.Specialized.OrderedDictionary _predictiveSearchCache = new System.Collections.Specialized.OrderedDictionary();

    private delegate void UpdateCache(string eventArgument);

    public string PredictiveSearchIndexesAlternate = "";

    public void RaiseCallbackEvent(string eventArgument)
    {
        //"ServicesIndex;PeopleIndex;Experience;News;Articles;Newsletters;Events;Webinars;WhitePapers;Awards;CLEs"

        var curPage = Request.RawUrl.Split('?')[0].ToLower();
        if (curPage.EndsWith("/services"))
            PredictiveSearchIndexes = "ServicesIndex";
        if (curPage.EndsWith("/") || curPage.EndsWith("/people"))
        {
            PredictiveSearchIndexes = "PeopleIndex";
        }
        if (curPage.EndsWith("/") || curPage.EndsWith("/people/search-results") || (curPage.EndsWith("/people") && !curPage.EndsWith("/search/people")))
        {
            PredictiveSearchIndexesAlternate = "2";
            PredictiveSearchIndexes = "PeopleIndex";
        }

        if (curPage.EndsWith("/work"))
            PredictiveSearchIndexes = "Experience";
        if (curPage.EndsWith("/insights"))
            PredictiveSearchIndexes = "WhitePapers;Webinars;Newsletters;Articles;News;Awards;Events";
        if (curPage.EndsWith("/newsletters"))
            PredictiveSearchIndexes = "Newsletters";
        if (curPage.EndsWith("/articles"))
            PredictiveSearchIndexes = "Articles";
        if (curPage.EndsWith("/news"))
            PredictiveSearchIndexes = "News";
        if (curPage.EndsWith("/awards"))
            PredictiveSearchIndexes = "Awards";
        if (curPage.EndsWith("/events"))
            PredictiveSearchIndexes = "Events";
        if (curPage.EndsWith("/white-papers"))
            PredictiveSearchIndexes = "WhitePapers";
        if (curPage.EndsWith("/cles"))
            PredictiveSearchIndexes = "CLEs";
        if (curPage.EndsWith("/webinars"))
            PredictiveSearchIndexes = "Webinars";
        if (curPage.EndsWith("/careers") || curPage.EndsWith("/current-opportunities"))
            PredictiveSearchIndexes = "CareersIndex";
        if (curPage.EndsWith("/firm"))
            PredictiveSearchIndexes = "TheFirm";
        ...
        if (!String.IsNullOrEmpty(eventArgument) && PredictiveSearchEnabled)
        {
            ...
            var cacheTerm = eventArgument + "_" + PredictiveSearchIndexes + "_" + PredictiveSearchIndexesAlternate;

            if (eventArgument == "clr psc")
                _predictiveSearchCache = new System.Collections.Specialized.OrderedDictionary();
            else if (eventArgument.StartsWith("rm psc"))
            {
                var clrTerm = eventArgument.Replace("rm psc", "").Trim();
                _predictiveSearchCache.Remove(clrTerm + "_" + PredictiveSearchIndexes + "_" + PredictiveSearchIndexesAlternate);
            }
            else if (eventArgument.StartsWith("upd psc"))
            {
                string[] keys = new string[_predictiveSearchCache.Count];
                _predictiveSearchCache.Keys.CopyTo(keys, 0);

                foreach (var key in keys)
                {
                    DataSet tmpSearchResults = PredictiveSearch(key);
                    mPredictiveSearchResult = RenderResults(tmpSearchResults, key);

                    _predictiveSearchCache[key] = new PredictiveSearchResult() { TimeStamp = DateTime.Now, Result = mPredictiveSearchResult };
                }
            }
            if (_predictiveSearchCache.Contains(cacheTerm))
            {
                var psResult = (PredictiveSearchResult)_predictiveSearchCache[cacheTerm];

                if (psResult.TimeStamp.AddDays(1) < DateTime.Now)
                {
                    mPredictiveSearchResult = psResult.Result;

                    _predictiveSearchCache.Remove(cacheTerm);

                    UpdateCache updCache = new UpdateCache(BackgroundWorker1_DoWork);
                    updCache.BeginInvoke(eventArgument, null, null);
            ...
            {
                DataSet searchResults = PredictiveSearch(eventArgument);
                mPredictiveSearchResult = RenderResults(searchResults, eventArgument);

                if (_predictiveSearchCache.Count > 1000)
                    _predictiveSearchCache.RemoveAt(0);


                _predictiveSearchCache.Add(cacheTerm, new PredictiveSearchResult() { TimeStamp = DateTime.Now, Result = mPredictiveSearchResult });
            }

            //var tmpResultAll = mPredictiveSearchResult.Split(new string[] { "<div class='predictiveSearchCategory nonSelectable'>" }, StringSplitOptions.RemoveEmptyEntries);
            //mPredictiveSearchResult = "";

            //var indexes = PredictiveSearchIndexes.Split(';').ToList();
            //foreach (var index in indexes)
            //{
            //    var find = index.ToLower().Replace("index", "") + "</div>";
            //    var result = tmpResultAll.SingleOrDefault(x => x.Substring(0, 30).ToLower().Replace(" ", "").StartsWith(find));
            //    if (!string.IsNullOrEmpty(result))
            //        mPredictiveSearchResult += "<div class='predictiveSearchCategory nonSelectable'>" + result;
            //}

            Response.Clear();
            Response.Write(mPredictiveSearchResult);
            Response.Flush();
    ...
    private void BackgroundWorker1_DoWork(string eventArgument)
    {
